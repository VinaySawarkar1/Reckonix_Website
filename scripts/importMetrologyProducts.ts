...
// Load products from metrology JSON file
const products = JSON.parse(fs.readFileSync('/home/ubuntu/Reckonix/meatest_metrology_products.json', 'utf-8'));
...
async function main() {
  ...
  for (const product of products) {
    ...
    // Set category for metrology products
    rest.category = 'Metrology Systems';
    
    // Set appropriate subcategory based on product type
    if (rest.name.toLowerCase().includes('vision') || rest.name.toLowerCase().includes('vm')) {
      rest.subcategory = 'Vision Measuring Machine';
    } else if (rest.name.toLowerCase().includes('cmm') || rest.name.toLowerCase().includes('coordinate')) {
      rest.subcategory = 'Coordinate Measuring Machine';
    } else if (rest.name.toLowerCase().includes('tool') || rest.name.toLowerCase().includes('presetter')) {
      rest.subcategory = 'Tool Presetter';
    } else if (rest.name.toLowerCase().includes('datalogger') || rest.name.toLowerCase().includes('logger')) {
      ...
      rest.subcategory = 'Metrology Equipment';
    }
...
main()
  .then(() => {
    console.log('Metrology product import/update complete!');
    process.exit(0);
  })
  .catch((e) => {
    console.error(e);
