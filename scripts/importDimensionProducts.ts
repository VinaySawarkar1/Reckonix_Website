...
// Load products from dimension calibration JSON file
const products = JSON.parse(fs.readFileSync('/home/ubuntu/Reckonix/meatest_dimension_products.json', 'utf-8'));
...
async function main() {
  ...
  for (const product of products) {
    ...
    // Set category for dimension calibration products
    rest.category = 'Calibration System';
    rest.subcategory = 'Dimension Calibrators';
  ...
  console.log(`\nDimension Calibration Import Summary:`);
...
main()
  .then(() => {
    console.log('Dimension calibration product import/update complete!');
    process.exit(0);
  })
  .catch((e) => {
    console.error(e);
